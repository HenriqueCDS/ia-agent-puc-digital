...
def _sem_pii(query: Query) -> Query:
    ...
    ORDEM (PII-3): chamada DEPOIS do guardrail e da triagem em `_responder` —
    as duas casam padrões sobre o texto original (um deles preso a "ID de
    aluno", que `pii.mascarar` consumiria) e não fazem egress. Antes só de
    retrieval/base/web.
