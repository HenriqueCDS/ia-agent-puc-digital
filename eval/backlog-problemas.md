...
| [x] | KB-2 | 🟠 | `WEB_ALLOWLIST` amplo demais — portal `puc-campinas.edu.br` inteiro + todos os subdomínios (vestibular, avaliação institucional, LPs, PDFs soltos) alucinava sobre assunto fora do agente (Q7 nota mínima citou vestibular) | `config.WEB_ALLOWLIST`, `web_fallback.fonte_permitida` | Feito: `learn.microsoft.com` já não estava; portal reduzido a `pucdigital.puc-campinas.edu.br` (subdomínio do programa, inteiro) + lista de `path_prefixes` curados no www (`/calendario/`, `/secretaria-geral/`, `/biblioteca/`). `FonteWeb.path_prefix` virou tupla `path_prefixes` | ✅ | 2026-08-28 |
| [~] | KB-3 | 🟡 | Fallback web custa ~50x o caminho da base (~15s vs ~300ms) | `scripts.lacunas`, `web_fallback` | Análise em `eval/analises/kb-3-melhorar-fallback-na-base.md`: (1) rotina semanal de indexar lacunas amarelas; (2) cache da camada de coleta web (TTL curto); (3) cortar 2ª rodada + baixar `WEB_SEARCH_TIMEOUT` após medir | 🔧 | análise 2026-08-28 |
...
- **2026-08-28** — KB-2: `WEB_ALLOWLIST` do portal PUC reduzida a `pucdigital.puc-campinas.edu.br` + `path_prefixes` curados (`/calendario/`, `/secretaria-geral/`, `/biblioteca/`); `FonteWeb.path_prefix` → tupla `path_prefixes`.
- **2026-08-28** — KB-3: análise de melhoria do fallback em `eval/analises/kb-3-melhorar-fallback-na-base.md` (parcial).
